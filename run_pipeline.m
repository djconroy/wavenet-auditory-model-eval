function run_pipeline(spl, snr)
    ...
    while hasdata(timitDatastore)
        ...
        parfor CFlp = 1:numCFs
            CF = CFs(CFlp);
            vihc = model_IHC_BEZ2018a(stim100k,CF,nrep,1/Fs,simdur,cohcs(CFlp),cihcs(CFlp),species);
            vihc_mat(CFlp,:) = vihc;

            for spontlp = 1:sum(numsponts)
                psth_ft = model_Synapse_BEZ2018a(vihc,CF,nrep,1/Fs,noiseType,implnt,...
                    sponts_concat(CFlp,spontlp),tabss_concat(CFlp,spontlp),...
                    trels_concat(CFlp,spontlp),expliketype);
                psth_mr = sum(reshape(psth_ft,psthbins,length(psth_ft)/psthbins));
